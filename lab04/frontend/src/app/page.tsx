    ...
    <main style={s.main}>
      ...
      {tab === "index" && (
        <div style={s.panel}>
            ...
            <div style={s.successBox}>
              ...
              <div style={s.fileResultList}>
                {indexResult.files?.map((f: any, i: number) => (
                  <div key={i} style={s.fileResultItem}>
                    {f.filename} → {f.chunks} chunk{f.chunks !== 1 ? "s" : ""}
                  </div>
                ))}
              </div>
            </div>
          ...
          {indexResult?.error && (
            <div style={s.errorBox}>⚠️ {indexResult.error}</div>
          )}
        </div>
      )}
      ...
      {tab === "eval" && (
        <div style={s.panel}>
          ...
          <p style={s.hint}>
            Provide questions with ground-truth relevant files. Metrics: Precision@K, Recall@K, MRR, LLM-as-judge.
          </p>

          <textarea
            value={evalJson}
            onChange={e => setEvalJson(e.target.value)}
            style={s.evalTextarea}
            spellCheck={false}
          />
          ...
          <button
            onClick={handleEval}
            disabled={evaluating}
            style={{ ...s.primaryBtn, opacity: evaluating ? 0.6 : 1 }}
          >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              ...
              <div style={s.metricsGrid}>
                {[
                  ...
                  <div key={m.label} style={s.metricCard}>
                    ...
                    <div style={{ ...s.metricValue, color: scoreColor(m.value * 5) }}>
              ...
              <div style={s.metaRow}>
                <span style={{ color: "#94a3b8", fontSize: "0.8rem" }}>
                  Evaluated {evalResult.metrics.num_examples} example{evalResult.metrics.num_examples !== 1 ? "s" : ""}
                </span>
              </div>
              ...
              {evalResult.results.map((r, i) => (
                <div key={i} style={s.evalResultCard}>
                  ...
                  <div style={s.evalMiniMetrics}>
                    ...
                    <span style={{ color: scoreColor(r.llm_judge.overall) }}>
                      Judge: {r.llm_judge.overall}/5
                    </span>
                  </div>
                  ...
                  {r.llm_judge.feedback && (
                    <div style={s.judgeFeedback}>💬 {r.llm_judge.feedback}</div>
                  )}
                </div>
              ))}
            </div>
